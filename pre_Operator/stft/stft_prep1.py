...
def plot_windowed_analysis():
    ...
    time_files = sorted(glob.glob('windowed_time_pos_*_sec.dat'))

    for time_file in time_files:
        # Extract window position from filename
        pos = time_file.split('_')[3]
        freq_file = f'windowed_freq_pos_{pos}_sec.dat'
        ...
        fig.tight_layout()
        fig.savefig(f'windowed_analysis_pos_{pos}_sec.png')
        plt.close(fig)
